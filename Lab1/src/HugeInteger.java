...
//            non zero value (in order to ignore leading zeros)
    ...
    HugeInteger add(HugeInteger h){
        if(this.isNegative && !h.isNegative) { //-this + h
            HugeInteger temp1 = this.copy();
            HugeInteger temp2 = h.copy();
            temp1.isNegative = false;
            HugeInteger subtract =  temp2.subtract(temp1);
            if(temp1.compareTo(temp2)==1)
                subtract.isNegative = true;
            return subtract;
        }
        if(!this.isNegative && h.isNegative) { //this + -h
            HugeInteger temp1 = this.copy();
            ...
            HugeInteger subtract =  temp1.subtract(temp2);
            if(temp1.compareTo(temp2)==-1)
                subtract.isNegative = true;
            return subtract;
        }
        ...
        int diff = longer.getLength()-shorter.getLength();
        int carry = 0;
        for (int i = longer.getLength()-1; i >= 0; i--) {
            if(i>=diff){
                answer.insert(0, (longer.number[i] + shorter.number[i-diff] + carry)%10);
                carry = (longer.number[i] + shorter.number[i-diff] + carry)/10;
            }else {
                answer.insert(0, (longer.number[i] + carry) % 10);
                carry = (longer.number[i] + carry) / 10;
            }
        }
        if(carry!=0)
        ...
        return new HugeInteger(answer.toString()); //return a new HugeInt with the "fixedSum" string
    }
    ...
    int compareTo(HugeInteger h) {
        if (this.isNegative && !h.isNegative) { //neg vs pos
            ...
            return 1;
        } else if (this.isNegative) { //neg vs neg
